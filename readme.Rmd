...
Load data
```{r load titanic data, warning=FALSE}
train <- read_csv("./Data/train.csv", col_types = "nffcfnffcnfc")
test <- read_csv("./Data/test.csv", col_types = "nffcfnffcnfc")
```
...
Divide the variables into numeric and categorical and visualize each group.
```{r}
numeric_vars <- train %>% 
...
categorical_vars <- train %>% 
  select(Survived, Pclass, Sex, Embarked, SibSp, Parch) %>% 
  names()
```
...
for (i in seq_along(numeric_vars)) {
  ...
  numeric_plots <- append(numeric_plots, list(p))
}

do.call(grid.arrange, c(numeric_plots, ncol = 2))
```

####total numbers of passengers travelling with their families.
...
PassengerID shows no correlations with any other variable. It is evenly distributed across passenger class and sex - the two strongest correlations with the target variable.
...
It is not apparent from the correlation map that age has any influence on survival. Children (and women) were more likely to be saved. From this information we can already infer that the relationship between survival and age is not linear as the likelihood of survival does not increase with age. Let us take a closer look at the distribution of survival rates across age for each sex.
...
The proportion of missing values in the variables "Embarked", "Fare" and "Age" is low enough to impute them.
...
- Remove PassengerID, Cabin, Ticket
...
set.seed(123)
train_folds <- vfold_cv(data = train, repeats = 2)
...
rec <- recipe(Survived ~ ., train) %>% 
  ...
  step_impute_knn(Age, neighbors = 3) %>% 
```
...
From the differences between mean and median as well as the box plot above we see few but significant outliers. 
Fare is also correlated with passenger family size. Let us look at the fares across different family sizes in each passenger class.

```{r echo=FALSE}
train%>% 
  filter(!is.na(Fare)) %>% 
  ggplot(aes(x = fct_inseq(SibSp),
             y = Fare,
             fill = factor(SibSp))) +
  geom_boxplot() +
  labs(title = "Per class fares for passengers travelling with family",
       y = "Fare",
       x = "Siblings/Spouse") +
  facet_wrap(~fct_inseq(Pclass)) +
  theme_minimal() +
  theme(legend.position = "none") 
```
...
train %>% 
  filter(!is.na(Fare)) %>% 
  ggplot(aes(x = fct_inseq(Parch),
             y = Fare,
             fill = fct_inseq(Parch))) +
  geom_boxplot() +
...
We can now examine some summary statistics to make a sound decision on how we want to impute.
...
train %>% 
  ...
  summarise(Fare_median = median(Fare, na.rm = TRUE),
            ...
            Count = n())
```
Due to the large standard deviation in class 1 we choose the median for imputation of fares. 
Again we will create a function for the imputation.
```{r Fare imputation, echo=TRUE}
...
train_imputed %>% 
  group_by(Pclass) %>% 
  summarise(Missing = sum(is.na(Fare))) %>% 
  kable("simple")
```

```{r echo=FALSE}
impute_fare(train_imputed) %>% 
  group_by(Pclass) %>% 
  summarise(Missing = sum(is.na(Fare))) %>% 
  kable("simple")
```
...
For the imputation of age, we will extract the titles from inside the name column. 
Then we will create title groups and calculate the mean and standard deviation for each, from which we will sample and impute. 
...
In this section we will encode categorical variables.
We will use the fastDummies package to make quick work of encoding all character and factor columns.
Also, we remove the first dummy column to avoid multicollinearity issues down the line.
```{r echo=TRUE}
