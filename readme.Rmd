...
```
We will deal with missing values later on. 
For now, we replace blank cells and those containing only spaces, as well as 0s in Fare with NA.
...
First, we will a plot a correlation map to get an idea of the relationship between the variables.
Note that the correlation coefficient is a measure of linear relationships.
```{r}
correlation_matrix <- train %>%
  select(!c(Name, Ticketc, Cabin)) %>%
...
```
We can infer that females - in general - are more likely to survive. Passenger class is also a good predictor as we see a large proportion of deceased female passengers in the 3rd class, although absolute numbers are roughly equal all three classes. For males the proportion of survivors is smaller irrespective of class. Male passengers in first class were much more likely to survive than in other classes. 
...
Passenger class and fare are both equally strong correlated with 
